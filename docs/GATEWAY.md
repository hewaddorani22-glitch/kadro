...
| Auth | keine | Supabase-JWT, plattformseitig erzwungen |
...
- `verify_jwt = true` — die Plattform weist alles ohne gültiges Token ab,
  bevor unser Code läuft.
- `consume_analysis_quota()` zählt pro Nutzer und Tag hoch und ist
  `security definer`. Das Limit selbst ist **kein Argument** der Funktion, damit
  ein Client es nicht hochsetzen kann.
...
```bash
npx supabase functions logs nutrition
```
